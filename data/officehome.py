...
class officehome(Dataset):
    ...
    def init(source_list, target_list, setting, task):
            ...
            for line in f.readlines():
                if int(line.split()[1]) in source_id:
                    source_data.append(line.split()[0].replace(\
                        '/data','/data/luoxiao/whx/CDAN/data'))
        ...
        if task == 'cross':
            ...
            print('Query Num: {}'.format(officehome.QUERY_DATA.shape[0]))
            print('Retrieval Num: {}'.format(officehome.RETRIEVAL_DATA.shape[0]))
            print('Source Train Num: {}'.format(officehome.TRAIN_S_DATA.shape[0]))
            print('Target Train Num: {}'.format(officehome.TRAIN_T_DATA.shape[0]))
